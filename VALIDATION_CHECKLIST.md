...
- [ ] Row-count parsing is sensitive to `usecols`: reading a raw File 1A with a restricted column set can silently retain a malformed row that a full-column, all-fields parse skips (confirmed for the 2020 and 2022 raw files, which each contain one row with an extra tab-delimited field). Table D1's "Raw rows" and "Form R records" columns must be generated with the same parsing method `src/data_prep.py` actually uses (full columns, strict skip on bad lines), not a faster verification script with `usecols` — the two methods can differ by 1 row. Confirm this by running `src/data_prep.py --reconcile` directly rather than trusting a one-off script.
