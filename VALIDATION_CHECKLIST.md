...
- [ ] Confirm the sample-size totals quoted in prose (e.g., "123,611 eligible development records," "39,967 eligible holdout records") equal the sums of the relevant Table 4 columns.
- [ ] Any raw, unfiltered file shape (`df.shape` straight off disk) that is reported anywhere must be labeled as raw. As of the removal of Table D1 (Appendix D no longer carries a raw-row reconciliation table by design decision), the raw → Form R → manufacturing → consolidated walk lives only in `src/data_prep.py --reconcile` and `notebooks/01_data_exploration.ipynb` — run one of these directly before publishing rather than assuming the synopsis text alone proves the counts reconcile.
...
## 6. Consolidation tie-break and raw-parse sensitivity (added after the duplicate-filing audit)

- [ ] Duplicate Form R filings for the same TRIFD + CAS NUMBER + CHEMICAL NAME within a reporting year are resolved with the deterministic rule in `src/data_prep.py`'s `consolidate()`: the action flag is the logical OR across all duplicate filings, and the authoritative release/unit values come from the filing with the highest DOCUMENT CONTROL NUMBER. This rule is documented in Appendix A, Table A1, and it is what actually produces Table 3 and Table 4 — do not let the synopsis describe a different or unspecified tie-break rule.
- [ ] `src/data_prep.py --reduction10` reproduces Table 4 (matched/zero-release/eligible/reduction-rate) for every 2020–2021 through 2023–2024 transition. Compare its output to Table 4 exactly, not just approximately.
- [ ] Row-count parsing is sensitive to `usecols`: reading a raw File 1A with a restricted column set can silently retain a malformed row that a full-column, all-fields parse skips (confirmed for the 2020 and 2022 raw files, which each contain one row with an extra tab-delimited field). If raw/Form R row counts are ever reported again anywhere in the synopsis, generate them with the same parsing method `src/data_prep.py` actually uses (full columns, strict skip on bad lines), not a faster verification script with `usecols` — the two methods can differ by 1 row. Confirm this by running `src/data_prep.py --reconcile` directly rather than trusting a one-off script.
